...
export function SimpleJobCard({ job, onClick }) {
  const isUrgentHiring = job?.jobPlan !== "Premium" && job?.jobPlan !== "Hot";
  const applicantCount = job?.JobApplications?.length || 0;

  return (
    <Card
      className="group relative m-4 w-full overflow-hidden cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-2xl max-w-md bg-gradient-to-br from-white via-blue-50/50 to-purple-50/50 border-2 border-transparent hover:border-blue-400/50"
