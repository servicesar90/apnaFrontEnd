...
export default function JobCard({job}) {
    ...
    return (
        <div
            key={job?.id}
            className="bg-white p-5 mt-4 rounded-xl shadow-sm border hover:shadow transition cursor-pointer"
            onClick={() => navigate(`/jobs/${job.id}`)}
        >
             <div className="text-orange-600 text-sm font-semibold mb-1">Urgently hiring</div>
